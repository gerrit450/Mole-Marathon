...
public class BugScript : MonoBehaviour
{
    ...
    void consumed()
    {
        ...
        if (distance <= 1.2f) // if the player is near a bug
        {
            switch(this.name) // look what type of bug it is
            {
                case "y":
                    Inventory.yellowBugs++;
                    gameObject.GetComponent<Renderer>().enabled = false;
                    gameObject.transform.position = new Vector3(-500, -500, -500);  //remove it from the sight, so it doesn't affect the counter
